...
const backupCodeSchema = z.object({
  code: z
    .string()
    .regex(
      /^[A-Fa-f0-9]{6}-[A-Fa-f0-9]{6}$/i,
      "Enter the backup code exactly as shown (e.g. A1B2C3-D4E5F6)",
    ),
});
...
): response is { twoFactorRequired: true; challengeToken: string } {
...
export function AuthPage() {
  ...
  const { mutateAsync: verifyTwoFactor, isPending: isTwoFactorPending } =
    useVerifyTwoFactorLogin();
  ...
  const backupCodeForm = useForm<BackupCodeValues>({
  ...
  async function handleBackupCode({ code }: BackupCodeValues) {
    try {
      await verifyTwoFactor({
        challengeToken: twoFactorChallenge,
        code,
  ...
  return (
    <Grid
      ...
      bg="brand.bg"
      color="ink"
      gridTemplateColumns={{ base: "1fr", lg: "1fr 440px" }}
    >
      {/* Left panel */}
      <Flex
        as="section"
      ...
      >
        ...
        <Box maxW="2xl">
          ...
          >
            Upload recordings, process transcripts in the background, extract
            the work that matters, and ask direct questions across past
            conversations.
          </Text>
        </Box>
        ...
        <Box w="full">
          {!twoFactorChallenge && (
            ...
            >
              ...
              <Button
                ...
                _hover={
                  mode === "register" ? { bg: "ink" } : { bg: "stone.100" }
                }
          ...
          {twoFactorChallenge ? (
            useBackupCode ? (
              <Box
                as="form"
                onSubmit={backupCodeForm.handleSubmit(handleBackupCode)}
                mt="6"
              >
                <Box display="flex" flexDir="column" gap="4">
                  <Field.Root invalid={!!backupCodeForm.formState.errors.code}>
                ...
                <Text mt="4" fontSize="sm" color="stone.600">
                  Found your device?{" "}
                  <ChakraLink
                    fontWeight="medium"
                    color="moss"
            ...
            >
              ...
              {registrationMessage && (
                <Text
                  ...
                  rounded="md"
                  borderWidth="1px"
                  borderColor="rgba(36,107,91,0.2)"
                  bg="rgba(36,107,91,0.1)"
                  px="3"
                  py="2"
                  fontSize="sm"
                  color="moss"
